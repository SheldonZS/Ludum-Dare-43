...
public class JukeboxManager : MonoBehaviour {
    ...
    public void SelectMusic(string music)
    {
        ...
        switch (music)
        {
            case "title":
                playing = music;
                customLooper.GetComponent<customLoop>().playCustomLoop(Resources.Load<AudioClip>("BGM/Main Title"),53.666f, 78.951f);
            ...
            case "intro":
                playing = music;
                customLooper.GetComponent<customLoop>().playCustomLoop(Resources.Load<AudioClip>("BGM/IntroTune"),5.902f, 11.892f);
            ...
            case "wandering":
                ...
                if (playing != "wandering")
                {
                    ...
                    + "\n\tI am a big fan of Yoshi's Island for the SNES and its soundtrack, so I was pleased when asked to make layering orchestration for this theme. Every successful sacrifice adds another instrument to the track. I always enjoy the comparison of a theme at the beginning and end of a game. Click Wandering again for the next variation!";
                }
                else
                {
                    baldrCount++;
                    AudioClip next;
                    if (baldrCount <= 5 && baldrCount > 0)
                    {
                         next = Resources.Load<AudioClip>("BGM/Wandering-" + baldrCount);
                    }
                    else
                    {
                        baldrCount = 0;
                         next = Resources.Load<AudioClip>("BGM/Wandering");
                    }
